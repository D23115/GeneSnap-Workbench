"""GL002 promoter-luciferase 项目的 SQLite 持久化。"""
...
class ReporterProjectSummary:
    ...
    revision: int
    construct_count: int
    workflow_type: str = "promoter_luciferase_reporter"


@dataclass(frozen=True, slots=True)
class StoredReporterProject:
    project_id: str
    gene_symbol: str
    species: str
    vector_name: str
    ...
    vector_design: ReporterVectorDesignResult
...
class SQLiteReporterProjectRepository:
    ...
    def initialize(self) -> None:
        ...
        with self._connection() as connection:
            connection.executescript(
                ...
                CREATE TABLE IF NOT EXISTS reporter_generated_artifacts (
                    artifact_id INTEGER PRIMARY KEY AUTOINCREMENT,
                    project_id TEXT NOT NULL,
                    artifact_json TEXT NOT NULL,
                    generated_at TEXT NOT NULL,
                    FOREIGN KEY(project_id) REFERENCES reporter_projects(project_id)
                );
                CREATE INDEX IF NOT EXISTS idx_reporter_artifacts_project
                ON reporter_generated_artifacts(project_id, artifact_id);
                """,
            )
    ...
    def create_project(
        ...
        *,
        ...
        snapshot: ReporterProjectSnapshot,
        created_at: datetime,
        folder_suffix: str = "RPT",
    ) -> None:
        if design.project_id != project_id or snapshot.project_id != project_id:
            raise ValueError("项目、reporter 设计和状态快照的 project_id 必须一致")
        if design.design_version_id != snapshot.active_design_version_id:
            raise ValueError("状态快照没有绑定当前 reporter 设计版本")
        if vector_design.design_version_id != design.design_version_id:
            raise ValueError("载体结果没有绑定当前 reporter 设计版本")
        try:
            with self._connection() as connection:
                connection.execute(
                    """
                    INSERT INTO reporter_projects (
    ...
    def load_project(self, project_id: str) -> StoredReporterProject:
        with self._connection() as connection:
            row = connection.execute(
                "SELECT * FROM reporter_projects WHERE project_id = ?",
                (project_id,),
            ).fetchone()
        if row is None:
        ...
        if not isinstance(design, ReporterDesignVersion):
            raise TypeError("数据库中的 reporter 设计类型无效")
        if not isinstance(vector_design, ReporterVectorDesignResult):
            raise TypeError("数据库中的 reporter 载体结果类型无效")
        if not isinstance(snapshot, ReporterProjectSnapshot):
            raise TypeError("数据库中的 reporter 项目快照类型无效")
        return StoredReporterProject(
    ...
    def list_projects(self) -> tuple[ReporterProjectSummary, ...]:
        with self._connection() as connection:
    ...
    def append_artifacts(self, project_id: str, artifacts: tuple[GeneratedArtifact, ...]) -> None:
        with self._connection() as connection:
            if connection.execute(
                "SELECT 1 FROM reporter_projects WHERE project_id = ?",
                (project_id,),
            ).fetchone() is None:
                raise KeyError(project_id)
            connection.executemany(
                """
                INSERT INTO reporter_generated_artifacts (
                    project_id, artifact_json, generated_at
                ) VALUES (?, ?, ?)
                """,
                tuple(
                    (project_id, dumps_record(item), item.generated_at.isoformat())
    ...
    def list_artifacts(self, project_id: str) -> tuple[GeneratedArtifact, ...]:
        with self._connection() as connection:
            rows = connection.execute(
                """
                SELECT artifact_json FROM reporter_generated_artifacts
                WHERE project_id = ? ORDER BY artifact_id
                """,
                (project_id,),
            ).fetchall()
        ...
        if not all(isinstance(item, GeneratedArtifact) for item in artifacts):
            raise TypeError("数据库中的 reporter 输出记录类型无效")
        return artifacts
