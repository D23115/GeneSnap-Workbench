"""SQLite persistence for SYN projects and append-only artifact history."""
...
from genesnap_workbench.domain.syn import SYNDesignVersion, SYNProjectSnapshot
...
class SQLiteSYNProjectRepository:
    ...
    def initialize(self) -> None:
        ...
        with self._connection() as connection:
            connection.executescript(
                """
                CREATE TABLE IF NOT EXISTS syn_projects (
                ...
                CREATE TABLE IF NOT EXISTS generated_artifacts (
                    ...
                    FOREIGN KEY(project_id) REFERENCES syn_projects(project_id)
                );

                CREATE INDEX IF NOT EXISTS idx_artifacts_project
                ON generated_artifacts(project_id, artifact_id);
                """,
            )
    ...
    def create_project(
        ...
        *,
        ...
        if design.design_version_id != snapshot.active_design_version_id:
            raise ValueError("状态快照没有绑定当前设计版本")
        try:
            with self._connection() as connection:
                connection.execute(
                    """
                    INSERT INTO syn_projects (
                        project_id, target_name, received_date, due_date,
    ...
    def load_project(self, project_id: str) -> StoredSYNProject:
        with self._connection() as connection:
            row = connection.execute(
                "SELECT * FROM syn_projects WHERE project_id = ?",
                (project_id,),
            ).fetchone()
        if row is None:
        ...
        if not isinstance(design, SYNDesignVersion):
            raise TypeError("数据库中的设计记录类型无效")
        if not isinstance(snapshot, SYNProjectSnapshot):
            raise TypeError("数据库中的项目快照类型无效")
        return StoredSYNProject(
    ...
    def list_projects(self) -> tuple[SYNProjectSummary, ...]:
        with self._connection() as connection:
            rows = connection.execute(
                """
                SELECT project_id, target_name, status, received_date, due_date,
                       project_folder, folder_suffix, revision
                FROM syn_projects
    ...
    def append_artifacts(
        ...
        with self._connection() as connection:
            exists = connection.execute(
                "SELECT 1 FROM syn_projects WHERE project_id = ?",
                (project_id,),
            ).fetchone()
            if exists is None:
                raise KeyError(project_id)
    ...
    def list_artifacts(self, project_id: str) -> tuple[GeneratedArtifact, ...]:
        ...
        if not all(isinstance(item, GeneratedArtifact) for item in artifacts):
            raise TypeError("数据库中的导出物记录类型无效")
        return artifacts
