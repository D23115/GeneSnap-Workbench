...
`0.3.3` 是当前可安装整合版本，窗口、任务栏、EXE、安装器和快捷方式使用同一套多尺寸图标。CKO/Flox 仅保留扩展接口；Broad 或 NCBI BLAST 不可用时，shRNA 会明确要求人工确认，不会伪装成已完成脱靶筛查。实验室载体必须由用户导入精确图谱并确认 protocol，软件通过不等于湿实验验证通过。Broad GPP 当前服务条款面向研究用途，商业使用可能需要另行许可。
